...
1. **システム設定** → **一般** → **ログイン項目と機能拡張** → **機能拡張を追加** → **Finder** で「パスワード付きZIP圧縮」が有効になっているか確認
2. Finder を再起動する
   ```bash
   killall Finder
   ```
...
3. パスワード入力ダイアログが表示されるので、パスワードと確認用パスワードを入力して **「圧縮」** を押す
4. 同じフォルダにパスワード付き `.zip` ファイルが作成される
...
| 機能 | 説明 |
|------|------|
| パスワード2欄入力 | 1画面でパスワードと確認を入力、タイプミスを防止 |
